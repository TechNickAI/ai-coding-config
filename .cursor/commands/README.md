...
By using symlinks, we maintain a single source of truth in `.claude/commands/` while making the commands available to both IDEs.
...
The YAML frontmatter is optional but recommended. Cursor will display the command in its slash command menu, with or without frontmatter.
